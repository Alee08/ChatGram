    ...
    case RADIOLINK_MSG_DATA_RSP: // 0x05
        {
            if (payload.size() != 1 || payload[0] != 1) {
                warning("(im871a) sending of telegram failed.\n");
            } else {
                verbose("(im871a) telegram sent ok\n");
            }
            received_command_ = msgid;
            sem_post(&command_wait_);
        }
...
void WMBusIM871A::sendTelegram(Telegram *t) {
    pthread_mutex_lock(&command_lock_);

    vector<uchar> msg(5);
    msg[0] = IM871A_SERIAL_SOF;
    msg[1] = RADIOLINK_ID;
    msg[2] = RADIOLINK_MSG_DATA_REQ; // RADIOLINK_MSG_WMBUSMSG_REQ;
    msg[3] = t->payload.size()+1;
    msg[4] = 0x78;

    for (size_t i=0; i<t->payload.size(); ++i) {
        msg.push_back(t->payload[i]);
    }

    sent_command_ = RADIOLINK_MSG_WMBUSMSG_REQ;
    verbose("(im871a) send telegram\n");
    serial()->send(msg);
